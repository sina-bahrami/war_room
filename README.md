...
These commands are the fastest path to run the project on a MicroK8s-style Kubernetes environment.
...
### 2. Build the existing images

```bash
docker build -t prompcorp-backend:local ./backend
docker build -t prompcorp-frontend:local ./frontend
```
...
### 4. Provide the unified JSON source

The recommended source mode is the unified snapshot file:

- `warroom_dashboard_data.json`

Copy it to the MicroK8s host path used by the backend and ingestor pods:

```bash
sudo mkdir -p /var/snap/microk8s/common/prompcorp-tender-intelligence
sudo cp ../warroom_dashboard_data.json /var/snap/microk8s/common/prompcorp-tender-intelligence/warroom_dashboard_data.json
```
...
Edit `k8s/secret.yaml` and set a real `POSTGRES_PASSWORD`, then apply it:
...
```bash
microk8s kubectl apply -f k8s/ingestion-job.yaml
...
The default Kubernetes config uses:

- `WARROOM_JSON_SOURCE_URL=/project-root/warroom_dashboard_data.json`
- PostgreSQL and Redis internal services
- `frontend` exposed via NodePort and optional Ingress

For the full production-oriented runbook, see [DEPLOYMENT_K8S.md](./DEPLOYMENT_K8S.md).
